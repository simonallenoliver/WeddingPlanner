...
public class UserController : Controller
{
    ...
    public IActionResult LoginUser(LogUser logAttempt)
    {
        ...
        // succesful login sets user ID in session and redirects to weddings page
        HttpContext.Session.SetInt32("UserId", dbUser.UserId);
        HttpContext.Session.SetString("Username", dbUser.FirstName);
        return RedirectToAction("Weddings", "Wedding");
    }

    // this page displays upon registration or login
    // we use session check here to prevent people who are not logged in from accessing the any pages - the login for
    // the session check is in our controllers folder
    [SessionCheck]
    [HttpGet("users/weddings")]
